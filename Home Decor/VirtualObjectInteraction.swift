...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    func didPinch(_ gesture: UIPinchGestureRecognizer) {
        if gesture.state == .changed {
            ...
            let hitTestResaults = self.sceneView.hitTest(touchLocation, options: nil)
            
            if let hitTest = hitTestResaults.first {
        ...
        let pinchScaleX = Float(gesture.scale) * trackedObject!.scale.x
        let pinchScaleY = Float(gesture.scale) * trackedObject!.scale.y
        let pinchScaleZ = Float(gesture.scale) * trackedObject!.scale.z
        
        trackedObject?.scale = SCNVector3(pinchScaleX, pinchScaleY, pinchScaleZ)
